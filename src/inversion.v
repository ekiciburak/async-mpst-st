...
Proof. intro a.
       ...
       - pinversion H1.
         + rewrite <- meqAp3 in H3, H6, H7.
           ...
           * apply apf_eqb_eq in Hcs. subst.
             apply merge_same_aeq in H3.
             inversion H3. subst. simpl in H. rewrite eqb_refl in H. easy.
           * assert((ApnA3 a0 n) <> b).
             { apply apf_eqb_neq. easy. } 
             symmetry in H3.
             assert(merge_apf_cont b (p & [|(l, s', w')|]) = merge_apf_cont b (p & [|(l, s', w')|])) by easy.
             ...
             * destruct HP as (c,(Ha,(Hb,(Hc,Hd)))).
               ...
               specialize(reOrg3 c s p s0 s'0 l s' w'0 w' H11 Ha Hd); intro HR.
               ...
               pfold.
                ...
                easy.
                easy. 
                rewrite Hg in H6. rewrite merge_merge in H6.
                apply IHa in H6; try easy.
                split. intros.
                apply actionExLF with (a := a1) in H6. 
                rewrite merge_merge. easy.
                easy.
                intros.
                apply actionExRF with (a := a1) in H6. easy.
                rewrite <- merge_merge. easy.
                simpl in H. rewrite orbtf in H.
                easy. 
             ...
             * destruct HP as (c,(Ha,(Hb,(Hc,Hd)))).
                ...
                specialize(reOrg3 c p s l s' s0 s'0 w' w'0 H11 Ha Hd); intro HR.
                destruct HR as (d,(He,(Hf,Hg))).
                rewrite Hg.
                pfold.
                ...
                rewrite Hc in H13.
                apply InMergeF in H13. easy. easy.
                left.
                ...
                rewrite Hc Hf in H6.
                rewrite <- merge_merge in H6.
                rewrite(st_eq (merge_apf_cont (apf_receive p l s' d) w'0)) in H6. simpl in H6.
                apply IHa in H6; try easy.
                rewrite merge_merge in H6.
                split. intros.
                apply actionExLF with (a := a1) in H6. 
                easy. easy.
                intros.
                apply actionExRF with (a := a1) in H6. easy. easy.
                simpl in H. rewrite orbtf in H. easy.
...
Lemma cpcpeq: forall c1 c2 p l1 l2 s1 s2 w1 w2,
  isInC c1 p = false ->
  isInC c2 p = false ->
  merge_cpf_cont c1 (p & [|(l1, s1, w1)|]) = merge_cpf_cont c2 (p & [|(l2, s2, w2)|]) ->
  l1 = l2 /\ c1 = c2 /\ s1 = s2 /\ w1 = w2.
Proof. intro c1.
       induction c1; intros.
       - simpl in H.
         rewrite orbtf in H.
...
Proof. intro c1.
       ...
       - rewrite(st_eq(merge_cpf_cont (cpf_receive s s0 s1 c1) (p & [|(l1, s2, w1)|]))) in H1. simpl in H1.
         ...
         + apply ppcneq in H6.
           ...
           assert((merge_cpf_cont (Apf2Cpf (ApnA3 a n)) (merge_cpf_cont c3 (p & [|(l2, s3, w2)|]))) =
                  (merge_cpf_cont (Cpf_merge (Apf2Cpf (ApnA3 a n)) c3) (p & [|(l2, s3, w2)|]))).
           ...
           apply InMergeFC. 
           rewrite H6a in H0.
           apply InMergeFC in H0. simpl in H0. rewrite orbtf in H0. easy. 
           case_eq n; intros.
           ++ easy.
           ++ rewrite <- InN; easy.
           easy. easy.
         ...
         + pose proof H2 as H2n.
           ...
           + apply apf_eqb_neq in H.
             ...
             assert(((merge_cpf_cont (Apf2Cpf (Apf_merge a2 (apf_receive p l2 s3 d))) w')) =
                     (merge_cpf_cont (Apf2Cpf a2) (p & [|(l2,s3, merge_apf_cont d w')|]))).
             { rewrite <- !mgApf2Cpf. rewrite <- merge_merge.
               rewrite(st_eq(merge_apf_cont (apf_receive p l2 s3 d) w') ). simpl. easy.
             }
             rewrite H12 in H9.
             ...
             assert((merge_cpf_cont (Apf2Cpf (ApnA3 a n)) (merge_cpf_cont (Apf2Cpf d) (p & [|(l2, s3, w2)|]))) =
                    (merge_cpf_cont (Apf2Cpf (Apf_merge (ApnA3 a n) d)) (p & [|(l2, s3, w2)|]))).
             { rewrite <- !mgApf2Cpf. rewrite <- merge_merge. easy. }
             rewrite H12 in H9.
             specialize(IHc1 (Apf2Cpf (Apf_merge (ApnA3 a n) d))  p l1 s2 w1 l2 s3 w2).
             apply IHc1; try easy.
             rewrite <- inAC. apply InMergeF. 
             rewrite HHc in H0.
              apply InMergeF in H0. easy. 
             apply refinementR4_mon.
       - simpl in H.
         ...
         + destruct H6 as (c3,(H6a,H6b)).
           ...
           assert((merge_cpf_cont (Bpf2Cpf (BpnB3 b n)) (merge_cpf_cont c3 (p & [|(l2, s3, w2)|]))) =
                  (merge_cpf_cont (Cpf_merge (Bpf2Cpf (BpnB3 b n)) c3) (p & [|(l2, s3, w2)|]))).
           { rewrite <- merge_mergeC. easy. }
           rewrite H2 in H9.
           apply IHc1 in H9.
           easy. easy. rewrite H6a in H0.
           apply InMergeFC.
           ...
           assert((merge_cpf_cont (Bpf2Cpf (Bpf_merge (Cpf2Bpf c2) (bpf_receive p l2 s3 b3))) w') =
                  (merge_cpf_cont c2 (p & [|(l2,s3, merge_cpf_cont (Bpf2Cpf b3) w')|]))).
           { rewrite <- mgBpf2Cpf. rewrite <- merge_mergeS.
             rewrite mgBpf2Cpf. 
             rewrite(st_eq(merge_bpf_cont (bpf_receive p l2 s3 b3) w')). simpl.
             rewrite bcId.
             rewrite mgBpf2Cpf. easy.
           }
           rewrite H2 in H9.
           apply IHc1 in H9. easy. easy. easy.
           case_eq n; intros.
           - easy.
           - rewrite <- InNS; easy.
           easy.
       ...
       - rewrite cpfend_cn in H1.
         pinversion H1. subst.
         rewrite <- meqAp3 in H6, H9. simpl in H.
         assert(c2 = (Apf2Cpf (ApnA3 a n))).
         { rewrite mgApf2Cpf in H6.
           apply cpcpeq in H6. easy.
           rewrite <- inAC.
           case_eq n; intros.
           - easy.
           - rewrite <- InN; easy.
...
Proof. intro c1.
       induction c1; intros.
       - rewrite(st_eq(merge_cpf_cont (cpf_receive s s0 s1 c1) (p & [|(l, s2, w)|]))) in H1. simpl in H1.
         pinversion H1. subst.
         rewrite <- meqAp3 in H6, H9, H10.
         simpl in H.
         rewrite orbtf in H.
         ...
         + apply ppcneq in H6; try easy.
           destruct H6 as (c3,(H7a,H7b)).
           rewrite H7b in H9.
           rewrite mgApf2Cpf in H9.
           assert((merge_cpf_cont (Apf2Cpf (ApnA3 a n)) (merge_cpf_cont c3 (p & [|(l, s', w')|]))) =
                 (merge_cpf_cont (Cpf_merge (Apf2Cpf (ApnA3 a n)) c3) (p & [|(l, s', w')|]))).
           { rewrite <- merge_mergeC. easy. }
           rewrite H in H9.
           apply IHc1 in H9; try easy.
           ...
           assert((merge_cpf_cont (Cpf_merge (Apf2Cpf (ApnA3 a n)) (cpf_receive s s0 s'0 c3)) w') =
                  (merge_cpf_cont (Apf2Cpf (ApnA3 a n)) (s & [|(s0, s'0, merge_cpf_cont c3 w')|]))).
           { rewrite <- merge_mergeC.
             rewrite(st_eq(merge_cpf_cont (cpf_receive s s0 s'0 c3) w')). simpl. easy.
           }
           rewrite H2.
           rewrite <- mgApf2Cpf.
           pfold.
           ...
           assert((merge_cpf_cont (Cpf_merge (Apf2Cpf (ApnA3 a n)) c3) w') = 
                  (merge_cpf_cont (Apf2Cpf (ApnA3 a n)) (merge_cpf_cont c3 w'))).
           { rewrite <- merge_mergeC. easy. }
           rewrite <- H3.
           easy.
           split. intro.
           apply actionExLF with (a := a0) in H9.
           rewrite <- merge_mergeC in H9.
           rewrite <- mgApf2Cpf in H9. easy.
           easy.
           intros.
           apply actionExRF with (a := a0) in H9. easy.
           rewrite <- merge_mergeC.
           ...
           rewrite <- inAC.
           apply InMergeFC in H0.
           destruct H0 as (H0a, H0b). 
           rewrite <- inAC in H0a. easy.
           apply InMergeFC in H0.
           destruct H0 as (H0a, H0b). 
           rewrite <- inAC in H0a. simpl in H0b. rewrite orbtf in H0b. easy.
           case_eq n; intros.
           ++ easy.
           ++ rewrite <- InN; easy.
         + specialize(cftoaf c2 w' Heq); intro Heeq.
           ...
           ++ apply apf_eqb_neq in H.
              ...
              apply HH in H6; try easy. clear HH.
              destruct H6 as [H6 | H6].
              * destruct H6 as (c3,(H7a,(H7b,(H7c,H7d)))).
                rewrite H7c.
                apply pneqq4 in H7d.
                ...
                assert((merge_cpf_cont (Cpf_merge (Apf2Cpf (ApnA3 a n)) (Apf2Cpf a3)) w') =
                       (merge_apf_cont (ApnA3 a n) (merge_apf_cont a3 w'))).
                { rewrite <- merge_mergeC.
                  rewrite !mgApf2Cpf. easy.
                }
                rewrite <- H3. easy.
                ...
                apply InMergeFC. split.
                rewrite <- inAC in H0.
                apply InMergeF in H0. rewrite <- inAC. easy.
                rewrite <- inAC.
                rewrite <- inAC in H0.
                apply InMergeF in H0. simpl in H0. rewrite orbtf in H0. easy.
                easy. easy.
              * destruct H6 as (c3,(H7a,(H7b,(H7c,H7d)))).
                apply pneqq4 in H7d.
                destruct H7d as (a3,(H7d,(H7e,H7f))).
                rewrite H7c H7f in H9.
                ...
                apply InMergeF.
                rewrite H7f in H7c.
                assert(isInA (ApnA3 a n) s = false) as Hnin.
                { case_eq n; intros.
                  ++ easy.
                  ++ rewrite <- InN; easy.
                }
                rewrite H7c in Hnin.
                apply InMergeF in Hnin. simpl in Hnin. rewrite orbtf in Hnin. easy.
                left. rewrite <- merge_merge. easy.
                rewrite H7c H7f in H10.
                rewrite <- merge_merge in H10.
                rewrite(st_eq (merge_apf_cont (apf_receive p l s' a3) w'0)) in H10. simpl in H10.
                ...
                intros.
                apply actionExRF with (a := a0) in H9. easy.
                rewrite <- merge_merge in H3. easy.

                easy. easy.
                case_eq n; intros.
                ++ easy.
                ++ rewrite <- InN; easy.
                rewrite <- inAC in H0. easy.
         apply refinementR4_mon.
       - rewrite(st_eq (merge_cpf_cont (cpf_send s s0 s1 c1) (p & [|(l, s2, w)|]))) in H1. simpl in H1.
         rewrite(st_eq (merge_cpf_cont (cpf_send s s0 s1 c1) w)). simpl.
         pinversion H1.
         subst.
         rewrite <- meqBp3 in H6, H9, H10.
         apply ppcbeq in H6; try easy.
         destruct H6 as [H6 | H6].
         + destruct H6 as (c3,(H7a,H7b)).
           rewrite H7a.
           assert((merge_cpf_cont (Cpf_merge (Bpf2Cpf (BpnB3 b n)) (cpf_send s s0 s'0 c3)) w') =
                  (merge_cpf_cont (Bpf2Cpf (BpnB3 b n)) (s ! [|(s0, s'0, merge_cpf_cont c3 w')|]))).
           { rewrite <- merge_mergeC.
             rewrite(st_eq(merge_cpf_cont (cpf_send s s0 s'0 c3) w')). simpl. easy.
           }
           rewrite H2.
           rewrite <- mgBpf2Cpf.
           specialize(ref4_b (upaco2 refinementR4 bot2)
           ...
           assert((merge_cpf_cont (Bpf2Cpf (BpnB3 b n)) (merge_cpf_cont c3 (p & [|(l, s', w')|]))) =
                  (merge_cpf_cont (Cpf_merge (Bpf2Cpf (BpnB3 b n)) c3) (p & [|(l, s', w')|]))).
           { rewrite <- merge_mergeC. easy. }
           rewrite H3 in H9.
           apply IHc1 in H9; try easy.
           left.
           ...
           assert((merge_cpf_cont (Cpf_merge (Bpf2Cpf (BpnB3 b n)) c3) w') =
                  (merge_cpf_cont (Bpf2Cpf (Bpf_merge (BpnB3 b n) (Cpf2Bpf c3))) w')).
           { rewrite <- !merge_mergeC.
             setoid_rewrite mgCpf2Bpf at 3.
             rewrite cbId.
             rewrite <- !merge_mergeS.
             rewrite <- mgCpf2Bpf.
             rewrite mgCpf2Bpf.
           ...
           split. intro.
           apply actionExLF with (a := a) in H9.
           rewrite <- merge_mergeC in H9.
           rewrite <- mgBpf2Cpf in H9. easy.
           easy.
           intros.
           apply actionExRF with (a := a) in H9. easy.
           rewrite <- merge_mergeC.
         ...
         + destruct H6 as (c3,(H7a,H7b)).
           ...
           assert(isInB (BpnB3 b n) s = false) as Hnin.
           { case_eq n; intros.
             ++ easy.
             ++ rewrite <- InNS; easy.
           }
           rewrite H7a in Hnin.
           rewrite InMergeFS in Hnin. simpl in Hnin.
           destruct Hnin as (Hnn,Hnm).
           rewrite Hnn Hnm. easy.
           rewrite <- !merge_mergeS in H9.
           rewrite(st_eq(merge_bpf_cont (bpf_receive p l s' c3) w'0)) in H9. simpl in H9.
           rewrite mgBpf2Cpf in H9.
           apply IHc1 in H9; try easy.
           rewrite <- mgBpf2Cpf in H9. rewrite <- merge_mergeS.
           left. easy.
           rewrite bcId. easy.
           rewrite H7a in H10.
           ...
           rewrite <- merge_mergeS in H9.
           setoid_rewrite mgBpf2Cpf in H9 at 1.
           rewrite(st_eq(merge_bpf_cont (bpf_receive p l s' c3) w'0)) in H9. simpl in H9.
           apply IHc1 in H9; try easy.

           split. intro.
           apply actionExLF with (a := a) in H9.
           rewrite <- mgBpf2Cpf in H9. easy. easy.
           intros.
           apply actionExRF with (a := a) in H9. easy.
           rewrite <- mgBpf2Cpf. easy.
           rewrite bcId. easy.
       ...
       - rewrite cpfend_cn in H1.
         ...
         subst. rewrite mgApf2Cpf in H9. easy.
         rewrite <- inAC.
         case_eq n; intros.
         ++ easy.
         ++ rewrite <- InN; easy.
