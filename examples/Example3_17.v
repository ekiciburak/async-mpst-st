...
Proof. unfold subltype.
       ...
       split.
       pcofix CIH.
       rewrite(st_eq(lt2st lT')). simpl.
       rewrite(coseq_eq((cofix next (xs : seq.seq (string * local.sort * local)) : coseq (string * local.sort * st) :=
        match xs with
        | [] => [||]
        | ((l1, s1, t1) :: ys)%SEQ => cocons (l1, s1, lt2st t1) (next ys)
        end) [("cont", I, lt_receive "p" [("success", I, lt_end); ("error", B, lt_end)]);
              ("stop", (), lt_receive "p" [("success", I, lt_end); ("error", B, lt_end)])])). simpl.
       rewrite(coseq_eq ((cofix next (xs : seq.seq (string * local.sort * local)) : coseq (string * local.sort * st) :=
           match xs with
           | [] => [||]
       ...
       rewrite(coseq_eq((cofix next (xs : seq.seq (string * local.sort * local)) : coseq (string * local.sort * st) :=
         ...
         [("success", I, lt_send "q" [("cont", I, lt_end); ("stop", (), lt_end)]);
          ("error", B, lt_send "q" [("cont", I, lt_end); ("stop", (), lt_end)])])). simpl.
       rewrite(coseq_eq((cofix next (xs : seq.seq (string * local.sort * local)) : coseq (string * local.sort * st) :=
         match xs with
         | [] => [||]
         | ((l1, s1, t1) :: ys)%SEQ => cocons (l1, s1, lt2st t1) (next ys)
       ...
       rewrite(coseq_eq(cocons ("stop", (), lt2st lt_end)
             ((cofix next (xs : seq.seq (string * local.sort * local)) :
                   coseq (string * local.sort * st) :=
                 match xs with
                 | [] => [||]
                 | ((l1, s1, t1) :: ys)%SEQ => cocons (l1, s1, lt2st t1) (next ys)
